...
class ViewModelFactory @Inject constructor(private val useCaseImpl: DomainUseCase) :
    ViewModelProvider.NewInstanceFactory() {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return when {
            modelClass.isAssignableFrom(MainViewModel::class.java) -> MainViewModel(useCaseImpl) as T
            modelClass.isAssignableFrom(PopularTeamViewModel::class.java) -> PopularTeamViewModel(
                useCaseImpl
            ) as T
            modelClass.isAssignableFrom(ExploreViewModel::class.java) -> ExploreViewModel(
                useCaseImpl
            ) as T
            modelClass.isAssignableFrom(TrophiesViewModel::class.java) -> TrophiesViewModel(
                useCaseImpl
            ) as T
